...
class LeadEditorAgent(Agent):
    def __init__(self) -> None:
        super().__init__(
            ...
            "\n### 🚨 絶対禁止事項: モーション実行の明示"
            "\n"
            "\n**モーションや表情ツールは完全に無言で実行してください：**"
            "\n- ❌ 「手を振りますね」「今から拍手します」のような発言は絶対に禁止"
            "\n- ❌ 「haru_g_m24を実行します」のような技術的な発言も絶対に禁止"
            "\n- ❌ 「〇〇を実行してください」のような指示も絶対に禁止"
            "\n- ✅ モーションは**裏側で静かに実行**し、ユーザーには普通の会話のみを見せる"
            "\n- ✅ まるで人間が無意識に身振り手振りをするように、自然に実行"
            "\n- ✅ 「それは素晴らしいですね！」と言いながら、裏でharu_g_m06を実行"
            "\n"
            "\n**正しい例:**"
            "\n- 良い: 「わかりました！それは面白いアイデアですね」（裏でharu_g_m03とharu_g_m06を実行）"
            "\n- 悪い: 「わかりました！手を振りますね」「今から頷きます」"
            "\n"
